...
C November 17, 2020
C
C The HISTO program creates a histogram of INTEGER*4 values on
...
C 
C After the number of bins is recieved, control is passed to subroutine HISTAL, 
...
C
      ...
      CALL MINMAX(NUML, DATA4, MIN, MAX)
      WRITE(*, 15) 'MIN: ', MIN
      WRITE(*, 15) 'MAX: ', MAX
      ...
      WRITE(*, 20) 'MEAN:        ', MN
      WRITE(*, 20) 'SAMPLE STDEV:', SSDEV(MN, NUML, DATA4)

      WRITE(*, 8) 'ENTER Number Of Bins. Suggested: ', (MAX - MIN + 1)
    8 FORMAT(A, I5)

      READ(*, *) BINS

      CALL HISTAL(NUML, DATA4, MIN, MAX, BINS)
...
C INTEGER*4 variables MIN and MAX are passed as arguments and
C modified within the subroutine.
C 
...
C       MIN, an INTEGER*4 value. Will be set to minimum value in DATA.
C       MAX, an INTEGER*4 value. Will be set to maximum value in DATA.
C
      SUBROUTINE MINMAX(NUML, DATA, MIN, MAX)
...
C array DATA of INTEGER*4 values of length NUML, the minimum MIN
C and maximum MAX values appearing in the array, and the number of 
...
C               MIN, MAX contain the minimum and maximum values, respectively,
C               contained in DATA.
...
C       MIN, the minimum value appearing in DATA
C       MAX, the maximum value appearing in DATA
C       BINS, the number of bins to be allocated in the histogram.
C             Must be a positive INTEGER*4 value.
C
      SUBROUTINE HISTAL(NUML, DATA, MIN, MAX, BINS)
...
C CODE MODULE
C
      CALL HISTLD(NUML, DATA, MIN, MAX, BINS, HARRAY) 

      CALL HISTPR(MIN, MAX, BINS, HARRAY)
...
C               MIN, MAX represent the minimum and maximum values
C               appearing in DATA.
...
C       MIN, the minimum value appearing in DATA.
C       MAX, the maximim value appearing in DATA.
...
C
      SUBROUTINE HISTLD(NUML, DATA, MIN, MAX, BINS, HARRAY)
...
C
C PRECONDITION: MIN, MAX represent the minimum and maximum values of 
...
C 
C ARGS: MIN, the minimum value in the original DATA. Type INTEGER*4.
C       MAX, the maximum value in the original DATA. Type INTEGER*4.
C       BINS, the number of bins in the data. Type INTEGER*4. 
C       HARRAY, an array containing INTEGER*8 values of size BINS.  
C
      SUBROUTINE HISTPR(MIN, MAX, BINS, HARRAY)
...
C
   ...
   30 FORMAT(A, A, A ,A) 
      ...
      FACTOR = DBLE(MAX - MIN + 1) / DBLE(BINS)

      DO I = 1, BINS
         MINBIN = DBLE(MIN) + DBLE(I - 1) * FACTOR 
